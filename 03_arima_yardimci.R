...
arima_run <- function(data,train,slide,cusmet) {
  
  
  train_startdate <- as.Date(c("2007-09-24"))
  #numyears_train <- 1
  numyears_train <- as.numeric(train)
  ...
  bc <- boxcox(Count ~., data=data_ts) 
  lambda <- bc$x[which.max(bc$y)]
  data[ ,"TransformedCount"] <-  c()
  
  
  if (lambda <= 0.2 && lambda >= -0.2) {
    data[ ,"TransformedCount"] <- log(data[ ,"Count"])
  } else {
  ...
  win <- as.numeric(slide)
  
  if (all(cusmet == "Aggresive")) {
    
    kk <- 0.5
    H <- 0.365
    
  }
  else if (all(cusmet == "Moderate")){
  ...
  while (test_startdate < test_enddate) {
    ...
    if (lambda <= 0.2 && lambda >= -0.2) {
      ...
      testdata[ ,"Observed"] <- exp(testdata[ ,"TransformedCount"])
    } else {
  ...
  return_data <- list("data" = data , "ts" = gls_fit_ts, "cusum" = cusum_result, "tsmodel" = model_ts, "arimaplot" = p1)
  return(return_data)
}
