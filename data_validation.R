...
best_colnames <- list(
  c("Date","date"),
  c("Count","Count","Count of patients","CountofPatients","Countofpatients","Count_of_patients","Count_of_patients","Count_of_Patients"),
  c("Holiday", "holiday")
)
...
input.sanity.check <- function(data) {
  ...
  required_column_names <- c("Date","Count")
  ...
  # 2. Check the "Count" column. All the values of that column should be in numeric format)
  if(is.numeric(data[ , "Count"]) == FALSE) {
    error_message <- paste(error_message, "All the values of Count column should be numeric and positive.\n\n")
  }
